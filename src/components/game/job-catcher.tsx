...
const JOBS = [
  { title: "Backend Eng", company: "Google", color: "#10b981" },
  { title: "Full Stack", company: "Meta", color: "#34d399" },
  { title: "Cloud Arch", company: "AWS", color: "#06b6d4" },
  { title: "AI/ML Eng", company: "OpenAI", color: "#a78bfa" },
  { title: "DevOps", company: "Stripe", color: "#f59e0b" },
  { title: "Data Sci", company: "Databricks", color: "#fbbf24" },
];
...
export function JobCatcher({ onExit }: { onExit?: () => void }) {
  ...
  const loop = useCallback((ts: number) => {
    ...
    // Draw background - BLACK to match website
    ctx.fillStyle = "#000000";
    ctx.fillRect(0, 0, W, H);

    // Draw subtle grid - very dim white
    ...
    // Draw catcher - EMERALD accent matching website
    if (g.started && !g.over) {
      const BW = 110 * dpr;
      const BH = 18 * dpr;
      const ly = H - 70 * dpr;
    ...
    <div className="fixed inset-0 z-[300] overflow-hidden bg-black">
        ...
        <div className="absolute inset-x-0 top-0 flex items-start justify-between px-5 pt-4 pointer-events-none">
          <div>
            <div className="flex items-baseline gap-2">
              <span className="text-[9px] text-emerald-400/50 tracking-[0.2em] uppercase font-mono">Score</span>
              <span className="text-2xl font-black tabular-nums font-mono" style={{ color: "#10b981", textShadow: "0 0 20px #10b98160" }}>
          ...
          <div className="text-center">
            <div className="text-[9px] text-zinc-600 tracking-[0.2em] uppercase font-mono">Level</div>
            <div className="text-lg font-black font-mono" style={{ color: "#a78bfa" }}>{hud.level}</div>
          </div>
          <div className="flex flex-col items-end gap-2 pointer-events-auto">
            <button onClick={onExit} className="text-[10px] px-3 py-1 rounded border border-zinc-800 text-zinc-600 hover:text-white hover:border-zinc-600 transition-colors font-mono tracking-widest">
              EXIT
            </button>
            <div className="flex gap-1.5 mt-1">
              {hearts.map((alive, i) => (
                <div key={i} className="w-3 h-3 rounded-full transition-all duration-300"
                  style={{ background: alive ? "#ef4444" : "#27272a", boxShadow: alive ? "0 0 8px #ef444480" : "none" }} />
              ))}
            </div>
          </div>
        </div>
      ...
      {screen === "title" && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-8 px-8">
          <div className="text-center">
            <div className="text-5xl font-black tracking-tight font-mono mb-2" style={{ color: "#10b981", textShadow: "0 0 40px #10b98170" }}>
              JOB CATCHER
            </div>
            <div className="text-zinc-500 text-sm font-mono tracking-wider">Catch offers · Dodge rejections</div>
          </div>
          <div className="flex flex-col gap-2 text-sm font-mono text-zinc-500 max-w-xs w-full">
            {[["Move", "Mouse / touch to steer"], ["Green cards", "Catch for points"], ["Red cards", "Dodge — lose a life"], ["Combos", "Stack for x8 bonus"]].map(([k, v]) => (
              <div key={k} className="flex gap-3">
                <span className="text-zinc-400 w-24 shrink-0">{k}</span>
                <span>{v}</span>
              </div>
            ))}
          </div>
          <button onClick={startGame} className="px-12 py-3 rounded-xl font-black text-lg font-mono tracking-widest transition-all hover:scale-105 active:scale-95"
            style={{ background: "linear-gradient(135deg, #059669, #10b981)", boxShadow: "0 0 40px #10b98140", color: "#fff" }}>
            START
          </button>
        </div>
      )}
      ...
      {screen === "over" && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-8 px-8"
          ...
          <div className="text-center">
            <div className="text-zinc-500 font-mono text-xs tracking-[0.3em] uppercase mb-3">Game Over</div>
            <div className="text-7xl font-black tabular-nums font-mono" style={{ color: "#10b981", textShadow: "0 0 50px #10b98170" }}>
              {hud.score.toLocaleString()}
            </div>
            <div className="text-zinc-500 font-mono text-sm mt-2">Final Score</div>
          </div>
          <div className="flex flex-col gap-3 w-full max-w-[220px]">
            <button onClick={startGame} className="w-full py-3 rounded-xl font-black font-mono tracking-widest transition-all hover:scale-105 active:scale-95 text-base"
              style={{ background: "linear-gradient(135deg, #059669, #10b981)", boxShadow: "0 0 30px #10b98150", color: "#fff" }}>
              PLAY AGAIN
            </button>
            {onExit && (
              <button onClick={onExit} className="w-full py-3 rounded-xl font-black font-mono tracking-widest text-base border border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-600 transition-all">
                EXIT
              </button>
            )}
          </div>
